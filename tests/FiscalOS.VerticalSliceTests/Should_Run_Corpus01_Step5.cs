...
public sealed class Should_Run_Corpus01_Step5
{
    ...
    [Fact] public void Explicit_semantic_curation_is_the_only_promotion_gate() { var c = Candidate(); var time = DateTimeOffset.Parse("2026-01-01T00:00:00+00:00"); var accepted = SemanticCuration.Decide(c, c, SemanticCurationDisposition.Accepted, "reviewed", "curator", time); var rejected = SemanticCuration.Decide(c, c, SemanticCurationDisposition.Rejected, "reviewed", "curator", time); var review = SemanticCuration.Decide(c, c, SemanticCurationDisposition.NeedsReview, "reviewed", "curator", time); Assert.NotNull(SemanticCuration.Promote(c, accepted.Decision!)); Assert.Null(SemanticCuration.Promote(c, rejected.Decision!)); Assert.Null(SemanticCuration.Promote(c, review.Decision!)); }
    [Fact] public void Semantic_curation_preserves_candidate_lineage() { var c = Candidate(); var d = SemanticCuration.Decide(c, c, SemanticCurationDisposition.Accepted, "explicit", "reviewer", DateTimeOffset.Parse("2026-01-01T00:00:00+00:00")).Decision!; var promoted = SemanticCuration.Promote(c, d)!; Assert.Equal(c.SemanticCandidateId, promoted.Candidate.SemanticCandidateId); Assert.Equal(c.Grounding.RawArtifactId, promoted.Candidate.Grounding.RawArtifactId); Assert.Equal(c.LegalScope, promoted.Candidate.LegalScope); }
