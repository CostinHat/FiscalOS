...
public enum SemanticAsOfStatus { Found, NotFound, Ambiguous }
public sealed record SemanticAsOfResult(SemanticAsOfStatus Status, AcceptedSemanticKnowledge? Knowledge = null);
...
public sealed class FileSemanticKnowledgeRepository : ISemanticKnowledgeRepository
{
    ...
    {
        var records = await ReadAsync(cancellationToken);
        var visible = records.Where(x => x.Decision.SemanticConceptId == conceptId && x.Decision.Disposition == SemanticCurationDisposition.Accepted && x.Decision.KnowledgeTime <= knowledgeAsOf && x.Decision.LegalValidTime is not null && x.Decision.LegalValidTime.ValidFrom <= legalDate && (x.Decision.LegalValidTime.ValidTo is null || legalDate <= x.Decision.LegalValidTime.ValidTo)).ToList();
        visible = visible.Where(x => !visible.Any(y => y.Decision.SupersedesSemanticDecisionId == x.Decision.SemanticCurationDecisionId)).ToList();
        return visible.Count switch { 0 => new(SemanticAsOfStatus.NotFound), 1 => new(SemanticAsOfStatus.Found, new AcceptedSemanticKnowledge(visible[0].Decision, visible[0].Candidate)), _ => new(SemanticAsOfStatus.Ambiguous) };
    }
