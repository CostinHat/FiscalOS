...
## FOS-0448 Classification Runtime Service Lifetime Acceptance Snapshot

FOS-0448 accepts the classification runtime service lifetime contract reviewed
in FOS-0447 without changing runtime composition.

Accepted:
- Classification runtime services remain singleton by design.
- `ClassificationEngine` lifetime matches its dependency graph.
- `RuleRegistry` lifetime matches immutable runtime rule registration.
- `ILegalBasisResolver` lifetime matches current stateless behavior.
- Current lifetimes do not introduce hidden mutable shared state.
- Current lifetimes do not introduce scope-capture risks.
- Current lifetimes do not introduce service locator behavior.
- Runtime composition remains deterministic.
...
## FOS-0449 Classification Runtime Composition Determinism Review Snapshot

FOS-0449 reviews classification runtime composition determinism without
changing runtime behavior.

Validated:
- Runtime composition is deterministic across equivalent service registrations.
- `ClassificationEngine` resolves the same dependency graph for equivalent
  service collections.
- `RuleRegistry` construction is deterministic because registered rules are
  captured into a list.
- Curated rule registration order remains deterministic.
- Custom rule registration remains deterministic under caller-defined
...
- Ordering assumptions remain limited to accepted contracts: service
  registration order, registry enumeration order for same priority and
...
## FOS-0451 Classification Default Curated Rule Set Review Snapshot

FOS-0451 reviews the default curated classification rule set without changing
runtime composition.

Validated:
- The default curated runtime rule set remains intentionally small.
- `MicroenterpriseClassificationRule` remains part of the default curated set.
- `VatPayerClassificationRule` remains part of the default curated set.
- `AlwaysPassRule` remains excluded from runtime composition.
- Curated rule inclusion remains explicit and code-owned in
  `AddClassificationRuntime(...)`.
...
## FOS-0454 Classification Custom Rule Extension Contract Acceptance Snapshot

FOS-0454 accepts the custom classification rule extension contract reviewed in
FOS-0453 without changing runtime behavior.

Accepted:
- `AddClassificationRule<TClassificationRule>()` remains the sole custom rule
  extension boundary.
- Custom rules integrate through the same `RuleRegistry` path as curated rules.
- Custom rule registration remains explicit, code-owned and part of runtime
  composition.
- No dynamic discovery exists.
- No configuration-driven registration exists.
- No classification plugin architecture exists.
- No runtime mutation of the rule set exists after composition.
- Runtime composition remains deterministic.
...
## FOS-0455 Classification Rule Evaluation Result Contract Review Snapshot

FOS-0455 reviews the classification rule evaluation result contract without
changing runtime behavior.

Validated:
- `RuleEvaluationResult` remains a rule-level outcome contract.
- Ownership remains with classification rule evaluation.
- It is not reused as classification-engine outcome identity.
- It is not reused as legal-basis identity; legal basis resolution consumes
  citations through `ILegalBasisResolver`.
- It is not reused as audit identity; audit graph nodes and edges remain
  separate audit model values.
- It remains immutable and deterministic as a sealed record with init-only
  citation data.
- `ClassificationEngine` consumes `RuleEvaluationResult` values but does not
...
## FOS-0458 Classification Decision Contract Acceptance Snapshot

FOS-0458 accepts the classification decision result contract reviewed in
FOS-0457 without changing runtime behavior.

Accepted:
- `ClassificationResult` remains the classification-engine outcome contract.
- Ownership remains with `ClassificationEngine` orchestration.
- `ClassificationResult` remains distinct from `RuleEvaluationResult`.
- `ClassificationResult` remains distinct from `DecisionLegalBasis`.
- `ClassificationResult` remains distinct from `ExplanationGraph` and audit
  graph values.
- `ClassificationResult` remains deterministic and immutable.
- `ClassificationEngine` owns `ClassificationResult` creation.
...
## FOS-0462 Resolution-Classification Collaboration Boundary Review and Acceptance Snapshot

FOS-0462 reviews and accepts a documentation-only collaboration boundary without
changing runtime behavior.
...
- Classification may consume supplied Resolution status, structural candidates,
  and correlation data only as context.
- Resolution owns policy, attempts, outcomes, corrections, audit, provenance,
  and evidence-package composition.
...
Still deferred:
- Runtime coupling or redesign between Resolution and Classification.
- Persistence, public API, graph infrastructure or traversal, AI/NLP,
  ingestion changes, and rule generation.
- Merging `ResolutionResult`, `DecisionLegalBasis`, `AuditGraph`, provenance,
  evidence, or classification outcome contracts.

## FOS-0463 Ingestion-Legal Knowledge Handoff Boundary Review Snapshot

FOS-0463 defines a documentation-only handoff from the narrow deterministic
Ingestion foundation to explicit Legal Knowledge curation without changing
runtime behavior.

Reviewed boundary:
- Ingestion may hand off successful, traceable raw-document candidate material
  only after raw identity and Ingestion traceability are complete.
- The handoff may include raw content or reference, raw identity/version,
...
- Ingestion retains source, raw identity, batch, replay, failure, audit, and
  provenance ownership; Legal Knowledge owns separate curation of accepted
  material.
...
Still deferred:
- Runtime handoff implementation, durable persistence, and public API exposure.
...
Accepted baseline:
- FiscalOS is an independent fiscal/legal knowledge product and platform with
  its own lifecycle, canonical data, capabilities, data/persistence lifecycle,
  stable interfaces, and consumer-independent evolution.
- FiscalOS is consumer-agnostic; the Consumer Independence Principle keeps
  consumer-specific adapters and orchestration outside canonical Domain and
  capability ownership.
- FiscalOS is API-first and must support future authorized commercial external
  consumption through a Public API. Public exposure, endpoint design and access
  model remain separate roadmap decisions.
- Production-grade operation requires durable persistence and an authoritative,
  versioned, explicitly curated fiscal/legal data lifecycle.
- A FiscalOS human interface, if implemented, is a first-party consumer of the
  same stable platform boundaries.
- Commercial services evolve incrementally against demonstrated demand; delivery
  emphasizes executable, evidence-backed vertical slices with real data and
  measurable outcomes.
- AI/NLP remains controlled and verifiable at the current stage. It cannot
  silently become authoritative Legal Knowledge, a rule, Classification, fiscal
  conclusion, legal basis, Evidence, or Audit. Any autonomy extension needs a
  separate architecture decision, sufficient evidence and risk-proportionate
  controls.

AR-02 relationship:
- AR-03 supersedes only the strategic product-positioning statements in AR-02
  section 11 that describe FiscalOS as an internal engine without independent
  product or Public API direction.
- AR-02's valid internal boundaries remain unchanged: small generic Legal Core;
  separate Ingestion, Legal Knowledge, Resolution and Classification;
  Runtime -> Domain; separate Explanation, Legal Basis, Evidence, Provenance
  and Audit contracts; and transversal Assurance/Traceability.

Current state and next objective:
- The current implementation remains a narrow deterministic in-memory
  foundation. Durable persistence, production source acquisition, Public API
  exposure, a human-facing interface and AI/NLP implementation do not exist yet.
- The next objective is to define and then deliver a narrow, measurable
  production-grade vertical slice: authoritative real source -> acquisition ->
  immutable raw preservation -> normalization -> version/effective-date
  handling -> explicit curation -> Legal Knowledge -> Resolution and/or
  supported Classification -> Explanation + legal basis + Evidence/Provenance
  -> durable persistence -> authorized API response.
- The source, first supported question, persistence/API/UI technologies,
  deployment topology, pricing, commercial packaging and AI model/vendor remain
  open. For each stage, resolve only the architectural, legal, operational and
  product decisions needed for that stage; use evidence from real vertical
  slices to inform later decisions.
